...
An earlier version of this project did exactly that: periodically
snapshotting `π_old ← θ` and continuing training on the same `D` for many
...
**What used to be `init_from_checkpoint` is no longer a choice.** `θ` must
start at `π_β`'s weights, because "`π_old = π_β`" and "`θ` continues from
`π_β`" have to be the same claim about the same run — starting `θ` from a
fresh random network wouldn't correspond to any real PPO window at all
(real PPO never resets its weights before an update). This closes what an
earlier version of this README called an open research fork; it isn't a
free choice for this analysis. `FixedDPPOTrainer` now requires
`prior_state_dict` and raises if it's missing.

# Phase 1 — PPO Exploitation Gap on a Stochastic Maze

This document is the complete, self-contained record of the project's
first phase: characterizing the PPO exploitation gap on a stochastic
30x30 maze with redundant paths. It covers the environment, the fixed-D
training setup, the full hyperparameter sweep, and the disagreement
investigation that followed it -- ending with why that investigation
motivated moving to a new environment for phase 2.

See the top-level [README](../README.md) for the general research
question, how to run this codebase, and where phase 2 picks up.
...
## First hypothesis: prior asymmetry — rejected

**Hypothesis.** Because `π_β` is imperfect, states where it initially
favored the wrong action by a wide margin should see PPO recover a
*smaller fraction* of that error — not just more residual error in
absolute terms, but a shrinking correction fraction as the initial error
grows.

**Method** (`scripts/analyze_prior_correction.py`). For every
disagreement-relevant state, compare `π_β(a*|s)` (before training) against
the best configuration's final `π_θ(a*|s)`, where `a*` is `π_D*`'s
preferred action. Define `C(s)` = the fraction of the gap to full
certainty on `a*` that PPO actually closed. Because `C(s)` is a ratio with
`prior_error = 1 - π_β(a*|s)` in the denominator, its variance explodes as
`prior_error → 0`; an unweighted fit is dominated by exactly the states
where the ratio is least trustworthy. The script corrects this with
weighted least squares (`weight = prior_error²`), checked for stability
across a threshold-sensitivity sweep, and controls for `D` coverage
throughout. Two versions of "how wrong was the prior" were tested:
`prior_error` itself, and the sharper `pi_beta_prob_gap` (the gap against
the *specific* action PPO ends up choosing, rather than all non-`a*`
probability lumped together).

**Result: no significant relationship, in either direction, for either
variable, after the correction.**

<div align="center">

| predictor | WLS coefficient | p-value |
|---|---|---|
| `prior_error` | −0.073 | 0.56 |
| `pi_beta_prob_gap` | +0.021 | 0.77 |
...
showed a stronger raw association (`r≈0.63`) — but it turned out to be
driven almost entirely by one specific, separate pattern: 11 of the 14
sharpest reversals (`π_β` already strongly favored `a*`, yet the final
policy moved decisively away from it) share the exact same action-pair
confusion (`π_D*` prefers RIGHT, the best configuration prefers LEFT —
directly opposite directions), clustered along one band of the maze with
noticeably sparser coverage. Not independent support for the asymmetry
hypothesis — a distinct pattern, not yet understood, kept on record rather
than pursued further here.

**Why coverage alone isn't the next thing to chase either.** `π_D*`
(empirical) is computed from nothing but `D`'s own counts — it has no
more information available to it than PPO does, and still reaches
`0.992`. Whatever the residual `~0.04` gap is, "not enough data" cannot be
...
### Tightening the definition: requiring both references to agree

Empirical `π_D*` is built from `D`'s own finite-sample MLE counts --
exactly the kind of estimate that gets less reliable at the same
low-coverage states this whole investigation is about. Checked directly:
of the states where the two `π_D*` definitions disagree with *each
other*, the best configuration's actual choice matched the true-restricted
(more trustworthy) action in the large majority of cases. A meaningful
share of what had been counted as "PPO's error" was, on inspection,
empirical-reference noise.

`scripts/analyze_policy_agreement.py` now computes both references in one
pass and requires **both** to independently flag a state as a
disagreement (`is_disagreement_strict`), with `severity_strict` the more
conservative (smaller) of the two references' own severities:

<div align="center">

| | 54 states (empirical alone) | 36 states (strict) |
|---|---|---|
| `pi_beta_prob_gap`, partial `r` | 0.218 | **0.200** |
| sparser-pair coverage, partial `r` | -0.435 | -0.332 |

</div>

`pi_beta_prob_gap` barely moved under the stricter, cleaner definition --
the most robust finding of the two. Pair-level coverage weakened somewhat
(some of that signal was riding on the 18 states the stricter definition
now excludes) but remains real and the second-strongest factor.

### Validating against the training trajectory

Before trusting `π_D*` (true-restricted) as the cross-check reference,
its own disagreement calls needed to line up with something independent:
the actual training dynamics. `scripts/analyze_disagreement_trajectory.py`
reuses the 61 already-saved per-epoch checkpoints (no retraining) and
re-runs the same forward-pass recording against the new, stricter
36-state list.

The result reproduces the original 54-state finding almost exactly: epoch
0 (`π_β` itself, before any fixed-D training) is the peak of agreement
(`0.306`), declining to a trough of `0.0` around epoch 150-170 -- which
is, pointedly, within a few checkpoints of the best *aggregate* success
rate (epoch 155, `0.954`) -- before a partial, incomplete recovery to
`~0.11` by epoch 300. The shape survives the cleanup; it was not an
...
show this is not one uniform mechanism: many states start near-certain
(`π_β` confidently correct) and collapse hard and *permanently* within
the first 25-50 epochs; others oscillate noisily throughout, consistent
with the two-attractor oscillation documented earlier in "Epoch-count
ceiling analysis".

**A methodological worry, checked and resolved.** The best aggregate
checkpoint coinciding with *zero* agreement on these 36 states raised an
uncomfortable question: does disagreeing with `π_D*` here actually *help*
performance? Cross-referencing the full 61-epoch trajectory against
aggregate success_rate: the raw correlation across the whole run is
`-0.41`, but restricted to epoch ≥ 50 (once training has left its initial
rapid-improvement transient) it drops to `+0.07` -- essentially nothing.
The apparent inverse relationship is a coincidence of two unrelated
processes sharing the same early epochs (aggregate performance climbing
fast; agreement on these specific sparse states declining fast), not a
real trade-off.

### Does any of this actually matter for measured performance?

Resolving that correlation was not the end of the concern. The 36 strict
states were identified as a diagnostic construct; whether they are even
*reached* during a normal evaluation rollout -- the only thing
`success_rate` actually measures -- had never been checked.

`scripts/analyze_disagreement_state_visitation.py` replays the exact same
500-episode, seed-24680 rollout used to report this checkpoint's own
`success_rate` everywhere else in this project (sanity-checked: the
replayed rollout reproduces `0.954` exactly), additionally recording
which of the 36 states each episode actually visits.

<div align="center">

| | count |
|---|---|
| never visited in 500 episodes | 27 / 36 |
| visited in most episodes (97-99%) | 3 / 36 |
| visited occasionally (0.2-3.4%) | 6 / 36 |

</div>

Three-quarters of the states this whole diagnostic thread is built on
never come up in practice at all. Cross-referencing severity against
visitation sharpens this further: of the 12 states with real severity
(`> 0.3`), all but one are in the never- or barely-visited group. Exactly
one state -- visited in `3.4%` of episodes, `severity = 0.586` -- is both
meaningfully reached and meaningfully wrong. The three heavily-visited
states (`386`, `197`, `387`) are real, confirmed disagreements (both
references agree with each other, not reference noise) but have *low*
severity (`0.04-0.05`).

A closer look at those three explains why, and reframes the mechanism
again. At all three, `D` shows PPO's (wrong) chosen action **5 to 100
times more often** than `π_D*`'s preferred one (`2766-2878` samples vs.
`26-600`) -- not a sparse-signal story at all. Checked against the
training trajectory: all three show `agrees = 0` at *every single*
recorded checkpoint from epoch 0 through 300, with no exception. `π_β`
itself, before fixed-D training or even `D` collection ever began,
already preferred the same wrong direction. `D` faithfully inherited that
skew in its sample composition, and PPO faithfully extracted what `D`
actually shows. This is not PPO's mechanism failing to extract from `D`
-- it is `D` itself encoding a bias already present in `π_β`'s own
behavior, upstream of anything PPO's training touches. By the same logic
that ruled out recalibrating `π_β`'s critic (see "Policy agreement"
above), this is out of scope for a study of PPO's own extraction
mechanism.


## Why we're moving to a new environment

Put together, the last two sections point at the same uncomfortable
conclusion from two different directions: **the maze, as a task, does not
cleanly separate "PPO's mechanism failed to extract from `D`" from other
explanations.**

**`π_D*` (empirical) is not a safe, unbiased ground truth by
construction.** Value iteration is a purely greedy, unregularized
consumer of `D`'s raw counts -- an `(s,a)` pair backed by two lucky
samples is treated as confidently as one backed by two thousand. This
project already has direct evidence of the failure mode this produces:
the earlier hazard-distance masking check (see "Further analyses") showed
empirical `π_D*` collapsing to `0%` success when a narrow, thinly-sampled
route was masked, while true-restricted (exact dynamics, immune to MLE
noise) stayed near its ceiling. Greedy exploitation of finite samples is
its own kind of overfitting -- different from PPO's, but not obviously
more trustworthy as the standard PPO is being measured against.

**The maze's redundant paths absorb the cost of local disagreement.**
That same masking experiment showed geometric hazard-adjacency masks of
up to 67 states having *zero* effect on `π_D*`'s own success rate --
the maze reroutes around them for free. The visitation analysis above
shows the same thing from the opposite direction: the states where PPO
and `π_D*` genuinely disagree are overwhelmingly off the path a real
rollout ever takes, and the few that are on-path tend to be low-severity.
Disagreeing with a specific greedy-optimal action, in an environment built
with multiple viable routes, mostly costs nothing measurable.

Neither finding is a flaw in the analysis -- both are real, carefully
checked properties of this specific task. But together they mean that
**this maze cannot really distinguish "PPO extracts `D` well" from
"the task doesn't punish extracting `D` imperfectly."** Continuing to
refine factors and definitions on this environment would keep
characterizing a real phenomenon (directional bias inherited from `π_β`,
pair-level sparsity effects) without ever establishing whether it costs
